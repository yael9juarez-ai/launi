...
export default function InventoryPage() {
  ...
  const saveInventory = () => {
    ...
    toast({
      ...
      description: "Los cambios han sido guardados en el sistema central.",
    });
  };
  ...
  const resetToDefault = () => {
    ...
    toast({
      title: "🔄 REESTABLECIDO",
      description: "Inventario reiniciado a valores de fábrica.",
    });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <Card className="rounded-[2rem] border-none shadow-sm bg-white p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-black text-muted-foreground uppercase">Faltantes</p>
              <p className="text-2xl font-black">{items.filter(i => i.stock <= i.minStock).length} Críticos</p>
            </div>
          </div>
        </Card>
        <Card className="rounded-[2rem] border-none shadow-sm bg-white p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-black text-muted-foreground uppercase">Insumos Totales</p>
              <p className="text-2xl font-black">{items.length} Variedades</p>
            </div>
          </div>
        </Card>
      </div>
                  ...
                  <div className="flex items-center gap-3 justify-end md:w-1/4">
                    <div className="flex flex-col items-end mr-2">
                      ...
                      <div className="flex items-center bg-white rounded-2xl border-2 p-1 px-3 shadow-sm">
                        <Input 
                          type="number" 
                          className="w-24 border-none text-center font-black text-lg focus-visible:ring-0" 
                          value={item.stock} 
