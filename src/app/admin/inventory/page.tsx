...
import { ArrowLeft, Package, Save, AlertTriangle, Search, Utensils, Beaker } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const saveInventory = () => {
    toast({
      ...
      description: "Los niveles de stock han sido guardados correctamente.",
    });
  };
    ...
    <div className="min-h-screen bg-[#FDFDFD] p-4 md:p-8">
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-4xl font-black tracking-tighter">Control de Inventario</h1>
            <p className="text-muted-foreground font-medium">Gestiona existencias y disponibilidad.</p>
          </div>
        </div>
        ...
        <CardHeader className="p-8 pb-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <CardTitle className="text-2xl font-black flex items-center gap-2">
              <Package className="text-primary" /> Lista de Insumos
            </CardTitle>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b-2">
                ...
                <TableHead className="font-black text-xs uppercase tracking-widest">Categoría</TableHead>
                ...
                <TableRow key={item.id} className="hover:bg-muted/20 h-20">
                  ...
                  <TableCell>
                    <Badge variant="outline" className="rounded-full gap-1">
                      {item.category === 'Comida' ? <Utensils size={12} /> : <Beaker size={12} />}
                      {item.category}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {item.stock <= 5 ? (
                      ...
                      <Badge className="bg-emerald-500 text-white rounded-full">Óptimo</Badge>
                    )}
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" className="font-bold text-primary" onClick={() => handleStockChange(item.id, item.stock + 10)}>
                      +10
                    </Button>
                  </TableCell>
                </TableRow>
